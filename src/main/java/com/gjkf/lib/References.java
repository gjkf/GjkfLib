package com.gjkf.lib;

public class References {

	public static final String MODID = "gjkfLib";
	public static final String MOD_NAME = "gjkfLib";
	public static final String VERSION = "0.2";

	public static final String CLIENT_PROXY = "com.gjkf.lib.proxy.ClientProxy";
	public static final String COMMON_PROXY = "com.gjkf.lib.proxy.CommonProxy";
	public static final String SERVER_PROXY = "com.gjkf.lib.proxy.ServerProxy";
	
}