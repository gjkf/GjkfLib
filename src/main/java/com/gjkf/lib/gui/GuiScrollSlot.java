...
public abstract class GuiScrollSlot extends GuiWidget{    
	...
	public void mouseClicked(int mousex, int mousey, int button){
		...
		if(button == 0 && getScrollBarHeight() < height && mousex >= sbarx && mousex <= x + width && mousey >= y && mousey <= y + height){
			if(mousey < sbary){
			...
			//in the box
		}else if(mousex >= contentx && mousex < sbarx && mousey >= contenty && mousey <= contenty + contentheight){
