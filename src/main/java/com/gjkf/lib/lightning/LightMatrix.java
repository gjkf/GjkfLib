...
import com.gjkf.lib.vec.BlockCoord;
 ...
 */
public class LightMatrix implements GJRenderState.IVertexOperation{
    ...
    public boolean load() {
        ...
        GJRenderState.pipeline.addDependency(GJRenderState.lightCoordAttrib);
        return true;
    }

    @Override
    public void operate() {
        LC lc = GJRenderState.lc;
        float[] a = ao(lc.side);
        float f = (a[0] * lc.fa + a[1] * lc.fb + a[2] * lc.fc + a[3] * lc.fd);
        int[] b = brightness(lc.side);
        GJRenderState.setColour(ColourRGBA.multiplyC(GJRenderState.colour, f));
        GJRenderState.setBrightness((int) (b[0] * lc.fa + b[1] * lc.fb + b[2] * lc.fc + b[3] * lc.fd) & 0xFF00FF);
    }
