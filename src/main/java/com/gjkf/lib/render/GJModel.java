...
import com.gjkf.lib.lightning.LC;
import com.gjkf.lib.lightning.LightModel;
import com.gjkf.lib.render.uv.UV;
...
import static com.gjkf.lib.vec.Rotation.sideRotations;

public class GJModel implements GJRenderState.IVertexSource, Copyable<GJModel>
{
    ...
    protected GJModel(int vertexMode)
    {
    ...
    public Vector3[] normals() {
        return getAttributes(GJRenderState.normalAttrib);
    }
    ...
    @Override
    public <T> T getAttributes(GJRenderState.VertexAttribute<T> attr) {
    ...
    @Override
    public boolean hasAttribute(GJRenderState.VertexAttribute<?> attrib) {
    ...
    public <T> T getOrAllocate(GJRenderState.VertexAttribute<T> attrib) {
     ...
     */
    public GJModel generateBox(int i, double x1, double y1, double z1, double w, double h, double d, double tx, double ty, double tw, double th, double f){
        double u1, v1, u2, v2;
        double x2 = x1+w;
        double y2 = y1+h;
        double z2 = z1+d;
     ...
     */
    public GJModel generateBlock(int i, Cuboid6 bounds)
    {
        return generateBlock(i, bounds, 0);
    }

    public GJModel generateBlock(int i, Cuboid6 bounds, int mask)
    {
        return generateBlock(i, bounds.min.x, bounds.min.y, bounds.min.z, bounds.max.x, bounds.max.y, bounds.max.z, mask);
    }

    public GJModel generateBlock(int i, double x1, double y1, double z1, double x2, double y2, double z2)
    {
     ...
     * @return The generated model. When rendering an icon will need to be supplied for the UV transformation.
     */
    public GJModel generateBlock(int i, double x1, double y1, double z1, double x2, double y2, double z2, int mask)
    {
        double u1, v1, u2, v2;
     ...
     * @return The model
     */
    public GJModel computeNormals(int start, int length)
    {
        if(length%vp != 0 || start%vp != 0)
            throw new IllegalArgumentException("Cannot generate normals across polygons");

        Vector3[] normals = getOrAllocate(GJRenderState.normalAttrib);
     ...
     * @return The model
     */
    public GJModel computeLighting(LightModel light)
    {
        Vector3[] normals = normals();
        int[] colours = getAttributes(GJRenderState.lightingAttrib);
        if(colours == null) {
            colours = getOrAllocate(GJRenderState.lightingAttrib);
    ...
    public GJModel setColour(int c)
    {
        int[] colours = getOrAllocate(GJRenderState.colourAttrib);
     ...
     * @return The model
     */
    public GJModel computeLightCoords() {
        LC[] lcs = getOrAllocate(GJRenderState.lightCoordAttrib);
        Vector3[] normals = normals();
    ...
    public GJModel apply(Transformation t)
    {
        for(int k = 0; k < verts.length; k++)
            verts[k].apply(t);
    ...
    public GJModel apply(UVTransformation uvt)
    {
        for(int k = 0; k < verts.length; k++)
            verts[k].apply(uvt);
    ...
    public GJModel expand(int extraVerts)
    {
        int newLen = verts.length+extraVerts;
        verts = Arrays.copyOf(verts, newLen);
        for(int i = 0; i < attributes.size(); i++)
            if(attributes.get(i) != null)
                attributes.set(i, GJRenderState.copyOf((GJRenderState.VertexAttribute)GJRenderState.getAttribute(i), attributes.get(i), newLen));
    ...
    public void render(GJRenderState.IVertexOperation... ops)
    {
     ...
     */
    public void render(int start, int end, GJRenderState.IVertexOperation... ops)
    {
    	GJRenderState.setPipeline(this, start, end, ops);
    	GJRenderState.render();
    }

    public static GJModel quadModel(int numVerts)
    {
        return newModel(7, numVerts);
    }

    public static GJModel triModel(int numVerts)
    {
        return newModel(4, numVerts);
    }

    public static GJModel newModel(int vertexMode, int numVerts)
    {
        GJModel model = newModel(vertexMode);
        model.verts = new Vertex5[numVerts];
        return model;
    }

    public static GJModel newModel(int vertexMode)
    {
        return new GJModel(vertexMode);
    }
     ...
     * @throws IOException
     */
    public static Map<String, GJModel> parseObjModels(InputStream input, int vertexMode, Transformation coordSystem) throws IOException
    {
        if(coordSystem == null)
            coordSystem = new RedundantTransformation();
        int vp = vertexMode == 7 ? 4 : 3;

        HashMap<String, GJModel> modelMap = new HashMap<String, GJModel>();
     ...
     * @return A map of group names to models
     */
    public static Map<String, GJModel> parseObjModels(ResourceLocation res)
    {
     ...
     * @return A map of group names to models
     */
    public static Map<String, GJModel> parseObjModels(ResourceLocation res, Transformation coordSystem)
    {
        try
        {
     ...
     * @return A map of group names to models
     */
    public static Map<String, GJModel> parseObjModels(ResourceLocation res, int vertexMode, Transformation coordSystem)
    {
        try
        {
            return parseObjModels(
    ...
    public static GJModel createModel(List<Vector3> verts, List<Vector3> uvs, List<Vector3> normals, int vertexMode, List<int[]> polys)
    {
        int vp = vertexMode == 7 ? 4 : 3;
        ...
        GJModel model = GJModel.newModel(vertexMode, polys.size());
        if(hasNormals)
            model.getOrAllocate(GJRenderState.normalAttrib);
    ...
    public static void exportObj(Map<String, GJModel> models, PrintWriter p)
    {
        List<Vector3> verts = new ArrayList<Vector3>();
        List<UV> uvs = new ArrayList<UV>();
        List<Vector3> normals = new ArrayList<Vector3>();
        List<int[]> polys = new ArrayList<int[]>();
        for(Map.Entry<String, GJModel> e : models.entrySet()) {
            p.println("g "+e.getKey());
            GJModel m = e.getValue();
     ...
     */
    public GJModel shrinkUVs(double d)
    {
     ...
     */
    public GJModel sidedCopy(int side1, int side2, Vector3 point)
    {
     ...
     */
    public static void copy(GJModel src, int srcpos, GJModel dst, int destpos, int length)
    {
        ...
        for(int i = 0; i < src.attributes.size(); i++)
            if(src.attributes.get(i) != null)
            	GJRenderState.arrayCopy(src.attributes.get(i), srcpos, dst.getOrAllocate(GJRenderState.getAttribute(i)), destpos, length);
    }
     ...
     */
    public static void generateSidedModels(GJModel[] models, int side, Vector3 point)
    {
        for(int s = 0; s < 6; s++)
        {
            if(s == side)
     ...
     * @param point The rotation point
     */
    public static void generateSidedModelsH(GJModel[] models, int side, Vector3 point)
    {
        for(int s = 2; s < 6; s++)
        {
            if(s == side)
     ...
     * @return The model
     */
    public static GJModel generateBackface(GJModel src, int srcpos, GJModel dst, int destpos, int length)
    {
        int vp = src.vp;
        ...
        for(int i = 0; i < length; i++)
        {
            ...
            for(int a = 0; a < src.attributes.size(); a++)
                if(src.attributes.get(a) != null)
                	GJRenderState.arrayCopy(src.attributes.get(a), si, dst.getOrAllocate(GJRenderState.getAttribute(a)), di, 1);
     ...
     */
    public GJModel generateSidedParts(int side, Vector3 point)
    {
     ...
     */
    public GJModel generateSidedPartsH(int side, Vector3 point)
    {
     ...
     */
    public GJModel generateSidedPart(int side1, int side2, Vector3 point, int srcpos, int destpos, int length)
    {
     ...
     */
    public GJModel apply(Transformation t, int srcpos, int destpos, int length)
    {
        for(int k = 0; k < length; k++)
        {
    ...
    public static GJModel combine(Collection<GJModel> models)
    {
        if(models.isEmpty())
            return null;
        ...
        GJModel c_model = newModel(vertexMode, numVerts);
        int i = 0;
        for(GJModel model : models)
        {
    ...
    public GJModel twoFacedCopy()
    {
        GJModel model = newModel(vertexMode, verts.length*2);
    ...
    public GJModel copy()
    {
        GJModel model = newModel(vertexMode, verts.length);
        copy(this, 0, model, 0, verts.length);
        return model;
    }
    ...
    public GJModel zOffset(Cuboid6 offsets)
    {
        for(int k = 0; k < verts.length; k++)
        {
