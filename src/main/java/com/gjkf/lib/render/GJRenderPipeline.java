...
import com.gjkf.lib.render.GJRenderState.IVertexOperation;
import com.gjkf.lib.render.GJRenderState.VertexAttribute;

import java.util.ArrayList;

public class GJRenderPipeline
{
    public class PipelineBuilder
    {
        ...
        public PipelineBuilder add(IVertexOperation... ops) {
            for(int i = 0; i < ops.length; i++)
                GJRenderPipeline.this.ops.add(ops[i]);
            return this;
        }
    ...
    public void rebuild() {
        if(ops.isEmpty() || GJRenderState.model == null)
            return;

        //ensure enough nodes for all ops
        while(nodes.size() < GJRenderState.operationCount())
            nodes.add(new PipelineNode());
        unbuild();

        if(GJRenderState.useNormals)
            addAttribute(GJRenderState.normalAttrib);
        if(GJRenderState.useColour)
            addAttribute(GJRenderState.colourAttrib);
        if(GJRenderState.computeLighting)
            addAttribute(GJRenderState.lightingAttrib);
